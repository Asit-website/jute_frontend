...
const FABRICATOR_OPTIONS = ['Stitch & Sew Works', 'A1 Fabrication Hub', 'Quality Stitchers', 'Craft & Fabric Unit']

const COL_ISSUE = '110px 1.8fr 120px 100px 110px 1.5fr 90px'
const HEADS_ISSUE = ['Issue Date', 'Stitcher Name', 'PI No.', 'Item No', 'Qty Issued', 'Accessories Issued', 'Actions']
const MIN_W_ISSUE = '860px'

const COL_RECEIVE = '100px 1.4fr 110px 90px 90px 160px 1.2fr 120px 80px'
const HEADS_RECEIVE = ['Rec Date', 'Stitcher Name', 'PI No.', 'Item No', 'Qty Rec', 'Rejections (Fab/Fac)', 'Bill Details', 'QC Checked', 'Actions']
const MIN_W_RECEIVE = '1100px'

const emptyIssueRow = (defaultItemNo = '') => ({
  fabricatorName: 'Stitch & Sew Works',
  itemNo: defaultItemNo,
  qty: '',
  accessories: 'Thread & Piping',
  remarks: ''
})
...
export default function StitcherJob() {
  ...
  const [db, setDb] = useState({ stitcherJobs: { issues: [], receives: [] }, printerJobs: { receives: [] }, cuttings: [], pis: [] })
  ...
  const handleSaveIssue = () => {
    ...
    for (let idx = 0; idx < batchItems.length; idx++) {
      ...
      const totalIssuedStitch = db.stitcherJobs.issues
        .filter(s => s.id !== editId && s.piNo === batchPiNo && s.itemNo === item.itemNo)
        .reduce((sum, s) => sum + Number(s.qty || 0), 0)

      const available = stockAvailable - totalIssuedStitch

      if (Number(item.qty) > available) {
        setValidationError(`VALIDATION ERROR at row ${idx + 1}: Qty exceeds available panels! Max available for item "${item.itemNo}": ${available} pcs.`);
        return
      }
    }
    ...
    if (editId) {
      newDb.stitcherJobs.issues = newDb.stitcherJobs.issues.map(i => i.id === editId ? { ...savedItems[0], id: editId } : i)
    } else {
      savedItems.forEach(item => {
        const newId = 'ST-I-' + Date.now().toString().slice(-4) + Math.random().toString().slice(-2)
  ...
  const filteredIssues = (db.stitcherJobs?.issues || []).filter(i =>
    i.fabricatorName?.toLowerCase().includes(search.toLowerCase()) ||
    i.piNo?.toLowerCase().includes(search.toLowerCase()) ||
    i.itemNo?.toLowerCase().includes(search.toLowerCase())
  )

  const filteredReceives = (db.stitcherJobs?.receives || []).filter(r =>
    r.fabricatorName?.toLowerCase().includes(search.toLowerCase()) ||
            ...
            {tab === 0 ? (
              ...
              <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, p: 2, overflowX: 'auto' }}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: '200px 140px 120px 180px 1.5fr 36px', gap: 1.5, pb: 1, borderBottom: '1px solid', borderColor: 'divider', mb: 1.5, minWidth: '750px' }}>
                  {['Stitcher Name *', 'Item No *', 'Qty Issued *', 'Accessories Issued *', 'Remarks', ''].map(h => (
                ...
                <Stack spacing={1.5} sx={{ minWidth: '750px' }}>
                  {batchItems.map((row, idx) => (
                    <Box key={idx} sx={{ display: 'grid', gridTemplateColumns: '200px 140px 120px 180px 1.5fr 36px', gap: 1.5, alignItems: 'center' }}>
                      <TextField size="small" select value={row.fabricatorName} onChange={setRow(idx, 'fabricatorName')}>
                        {FABRICATOR_OPTIONS.map(f => <MenuItem key={f} value={f}>{f}</MenuItem>)}
                      </TextField>
                      ...
                      <TextField size="small" value={row.accessories} onChange={setRow(idx, 'accessories')} placeholder="Accessories" />
              ...
              <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, p: 2, overflowX: 'auto' }}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: '200px 140px 110px 90px 90px 110px 90px 110px 100px 120px 100px 36px', gap: 1.5, pb: 1, borderBottom: '1px solid', borderColor: 'divider', mb: 1.5, minWidth: '1300px' }}>
                  {['Stitcher Name *', 'Item No *', 'Qty Rec *', 'Rej Fab', 'Rej Fac', 'QC Checked', 'Bill Rec', 'Bill No', 'Bill Date', 'Bill Upload', 'Rej Remarks', ''].map(h => (
                    <Typography key={h} variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', fontSize: '0.62rem', textTransform: 'uppercase' }}>{h}</Typography>
                  ))}
                </Box>
