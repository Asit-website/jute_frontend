...
const initialData = [
  { id: 1, name: 'Raw Jute Fibre A-Grade', color: 'Natural', qty: 5000, uom: 'KG', rate: 85, status: 'Active' },
  { id: 2, name: 'Raw Cotton Yarn 40s', color: 'White', qty: 2500, uom: 'KG', rate: 145, status: 'Active' },
  { id: 3, name: 'Lamination Film Transparent', color: 'Clear', qty: 100, uom: 'ROLL', rate: 1200, status: 'Active' },
  { id: 4, name: 'Printing Ink - Jet Black', color: 'Black', qty: 350, uom: 'KG', rate: 450, status: 'Active' },
]

const UOMS = ['MTR', 'KG', 'PCS', 'ROLL', 'BAG', 'TON']
const statusColor = { Active: 'success', Inactive: 'error' }

const COL = '1.8fr 120px 120px 100px 120px 100px 100px'
const HEADS = ['Raw Material Name', 'Colour', 'Qty', 'Unit', 'Rate (₹)', 'Status', 'Actions']
...
export default function RawMaterialMaster() {
  ...
  const handleSave = () => {
    ...
    const rowData = {
      ...form,
      qty: form.qty === '' || form.qty === undefined ? '' : Number(form.qty),
      rate: Number(form.rate)
    }
    if (editId) {
      setItems(prev => prev.map(i => i.id === editId ? { ...rowData, id: editId } : i))
    } else {
      setItems(prev => [...prev, { ...rowData, id: Date.now() }])
    }
    setDialog(false)
  }
  ...
  return (
    <Box className="page-enter">
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>Maintain raw materials catalog</Typography>
          </Box>
        </Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          { label: 'Total Raw Materials', value: items.length, c: '#6C63FF', b: 'rgba(108,99,255,0.08)' },
          { label: 'Active Items', value: items.filter(i => i.status === 'Active').length, c: '#00C07F', b: 'rgba(0,192,127,0.08)' },
          { label: 'Total Value (₹)', value: `₹${items.reduce((s, i) => s + (Number(i.qty || 0) * Number(i.rate || 0)), 0).toLocaleString('en-IN')}`, c: '#F59E0B', b: 'rgba(245,158,11,0.08)' },
        ].map(s => (
      ...
      <Card>
          ...
          <Stack divider={<Divider />}>
            {filtered.map(i => (
              <Box key={i.id} sx={{ display: 'grid', gridTemplateColumns: COL, px: 2, py: 1.5, alignItems: 'center', '&:hover': { bgcolor: 'rgba(108,99,255,0.03)' }, transition: 'background 0.15s' }}>
                ...
                <Typography variant="body2" sx={{ fontWeight: 700, color: 'text.primary' }}>{i.qty}</Typography>
                <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600 }}>{i.uom}</Typography>
                <Typography variant="body2" sx={{ fontWeight: 700, color: '#374151' }}>₹{i.rate}</Typography>
        ...
        <Box sx={{ px: 2, py: 1.5, bgcolor: '#F8F9FC', borderTop: '1px solid', borderColor: 'divider' }}>
          <Typography variant="caption" sx={{ color: 'text.secondary' }}>Showing {filtered.length} of {items.length} raw materials</Typography>
        </Box>
      </Card>
        ...
        <DialogContent sx={{ pt: 2.5 }}>
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <Typography variant="caption" sx={{ fontWeight: 600, color: '#374151', mb: 0.5, display: 'block' }}>Colour</Typography>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <Typography variant="caption" sx={{ fontWeight: 600, color: '#374151', mb: 0.5, display: 'block' }}>Qty</Typography>
              <TextField fullWidth size="small" type="number" value={form.qty} onChange={f('qty')} placeholder="0" />
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <Typography variant="caption" sx={{ fontWeight: 600, color: '#374151', mb: 0.5, display: 'block' }}>Rate *</Typography>
              <TextField fullWidth size="small" type="number" value={form.rate} onChange={f('rate')} placeholder="0.00" />
            </Grid>
            <Grid size={12}>
